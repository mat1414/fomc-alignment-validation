"""
Export utilities for FOMC Alignment Validation Tool.
"""
import json
import numpy as np
import pandas as pd
...
def generate_results_json(
    ...
    filename = f"alignment_{ymd}_{coder_id}_{timestamp}.json"
...
def generate_results_csv(
    ...
    for key, val in validations.items():
        ...
        row = {
            ...
            "claude_alignment_score": alignment.get("claude_score"),
            "claude_alignment_justification": alignment.get("claude_justification"),
            "alignment_quotes_accurate": alignment.get("quotes_accurate"),
            "alignment_supports_interpretation": alignment.get("supports_interpretation"),
            "human_alignment_score": alignment.get("human_score"),
            "notes": val.get("notes", ""),
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"alignment_{ymd}_{coder_id}_{timestamp}.csv"
...
def restore_from_uploaded_json(uploaded_file) -> Tuple[bool, str, Optional[Dict]]:
    ...
    try:
        ...
        if metadata.get('validation_type') and metadata.get('validation_type') != 'alignment':
            return False, "This file contains influence validations, not alignment", None
