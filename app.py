...
A Streamlit application for validating LLM-extracted alignment and influence
scores from Federal Reserve FOMC meeting transcripts.
"""
...
# Page configuration
st.set_page_config(
    page_title="FOMC Alignment/Influence Validation",
    page_icon="📊",
    layout="wide",
...
def get_or_create_validation(
    ...
    if key not in st.session_state.validations:
        st.session_state.validations[key] = {
            ...
            'speaker': speaker,
            'alignment': {
                'claude_score': alignment_data.get('score') if alignment_data else None,
                'claude_justification': alignment_data.get('justification', '') if alignment_data else '',
                'quotes_accurate': None,
...
def render_sidebar(data: Dict):
    """Render the sidebar."""
    with st.sidebar:
        st.title("📊 Alignment/Influence Validation")
...
def render_assessment_form(
    ...
    validation = get_or_create_validation(
        decision_idx, speaker, decision, alignment_data, influence_data
    )

    st.markdown("---")

    # Two-column layout for Alignment and Influence
    col_align, col_influence = st.columns(2)

    # ALIGNMENT COLUMN
    with col_align:
        st.markdown("### Alignment")

        align = validation['alignment']
        claude_align_score = align.get('claude_score')

        if claude_align_score is not None:
            score_label = ALIGNMENT_SCALE.get(int(claude_align_score), "Unknown")
            st.metric("Claude's score", f"{claude_align_score} ({score_label})")
        else:
        ...
        st.text_area(
            ...
            value=align.get('claude_justification', 'No justification provided'),
            height=150,
            disabled=True,
            key=f"align_just_{decision_idx}_{speaker}",
            label_visibility="collapsed"
        )
        ...
        # 1. Quotes accurate?
        st.markdown("1. Quotes/evidence accurate?")
        quotes_accurate = st.radio(
            "Quotes accurate",
            options=["Yes", "Partially", "No"],
            index=["yes", "partially", "no"].index(align.get('quotes_accurate')) if align.get('quotes_accurate') in ["yes", "partially", "no"] else None,
            key=f"quotes_acc_{decision_idx}_{speaker}",
            horizontal=True,
            label_visibility="collapsed"
        )
        if quotes_accurate:
            align['quotes_accurate'] = quotes_accurate.lower()

        # 2. Supports interpretation?
        st.markdown("2. Supports interpretation?")
        supports_interp_align = st.radio(
            "Supports interpretation (align)",
            options=["Yes", "Partially", "No"],
            index=["yes", "partially", "no"].index(align.get('supports_interpretation')) if align.get('supports_interpretation') in ["yes", "partially", "no"] else None,
            key=f"supports_align_{decision_idx}_{speaker}",
            horizontal=True,
            label_visibility="collapsed"
        )
        if supports_interp_align:
            align['supports_interpretation'] = supports_interp_align.lower()

        # 3. Your alignment score
        st.markdown("3. Your alignment score:")

        with st.expander("Scale reference", expanded=False):
            for score, desc in ALIGNMENT_SCALE.items():
                st.caption(f"{score:+d}: {desc}")

        human_align_score = st.select_slider(
            "Your alignment score",
            options=[-3, -2, -1, 0, 1, 2, 3],
            value=align.get('human_score') if align.get('human_score') is not None else 0,
            format_func=lambda x: f"{x:+d}",
            key=f"human_align_{decision_idx}_{speaker}",
            label_visibility="collapsed"
        )
        align['human_score'] = human_align_score
    ...
    with col_influence:
        ...
        # 1. Evidence supports score?
        st.markdown("1. Evidence supports score?")
        supports_interp_inf = st.radio(
            "Supports interpretation (inf)",
            options=["Yes", "Partially", "No"],
        ...
        if supports_interp_inf:
            inf['supports_interpretation'] = supports_interp_inf.lower()

        # 2. Your influence score
        ...
        human_inf_score = st.radio(
            ...
            format_func=lambda x: f"{x} - {INFLUENCE_SCALE[x].split('(')[0].strip()}",
    ...
    with col2:
        if st.button("✅ Submit & Next →", type="primary", use_container_width=True):
            ...
            missing = []
            if align.get('quotes_accurate') is None:
                missing.append("Alignment: Quotes accurate")
            if align.get('supports_interpretation') is None:
                missing.append("Alignment: Supports interpretation")
            if inf.get('supports_interpretation') is None:
                missing.append("Influence: Supports interpretation")
...
def main():
    ...
    # Ensure speaker index is valid
    if st.session_state.current_speaker_idx >= len(speakers):
        st.session_state.current_speaker_idx = 0

    # Render main content
    st.title("📊 FOMC Alignment & Influence Validation")
