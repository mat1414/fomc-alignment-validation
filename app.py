...
A Streamlit application for validating LLM-extracted alignment scores
...
st.set_page_config(
    page_title="FOMC Alignment Validation",
    page_icon="📊",
    layout="wide",
...
def render_assessment_form(
    ...
    """Render the alignment assessment form."""
    validation = get_or_create_validation(
        decision_idx, speaker, decision, alignment_data
    )
    ...
    if supports_interp:
        align['supports_interpretation'] = supports_interp.lower()
    ...
    with col2:
        if st.button("Submit & Next", type="primary", use_container_width=True):
            ...
            if align.get('quotes_accurate') is None:
                missing.append("Quotes accurate")
            if align.get('supports_interpretation') is None:
                missing.append("Supports interpretation")
...
def main():
    ...
    st.title("FOMC Alignment Validation")
